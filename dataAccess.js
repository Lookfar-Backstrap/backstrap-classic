...
DataAccess.prototype.updateEntity = function (updateObj, user, connection, callback) {
  ...
  // INTO PROPERTIES, OBJ/ARRAY PROPERTIES, NOSQL PROPERTIES, AND TO-ONE RELATIONSHIPS
  var updateProps = {};
  var updateObjProps = {};
      ...
      if(arg === prop.name) {
        if(prop.nosql === true) {
          ...
          if(prop.data_type === 'object' || prop.data_type === 'array') {
            updateObjProps[arg] = updateObj[arg];
          }
          else {
  ...
  // HANDLE THE UPDATE PROPERTIES WHICH ARE JSONB DB COLUMNS
  let upObjKeys = Object.getOwnPropertyNames(updateObjProps);
  upObjKeys.forEach((uok, idx) => {
    ...
    qry += uok+" = '"+JSON.stringify(updateObjProps[uok])+"'";
  });
  ...
  toOneRelKeys.forEach((rk, idx) => {
    if(idx === 0 && updateProps.length === 0 && 
        updateObjProps.length === 0 && updateNoSqlProps.length === 0) {
      qry += " ";
    }
    else {
      qry += ", ";
